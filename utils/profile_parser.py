...
class ProfileParser:
    ...
    def parse(self) -> UserProfile:
        total_atk = (
            self._extract(StatsPatterns.BASIC_ATTACK, "basic_attack") 
            + self._extract(StatsPatterns.EXTRA_ATTACK, "extra_attack")
        )
        total_arm = (
            self._extract(StatsPatterns.BASIC_ARMOR, "basic_armor") 
            + self._extract(StatsPatterns.EXTRA_ARMOR, "extra_armor")
        )
        total_dodge = (
            self._extract(StatsPatterns.BASIC_DODGE, "basic_dodge") 
            + self._extract(StatsPatterns.EXTRA_DODGE, "extra_dodge")
        )
        total_crit = (
            self._extract(StatsPatterns.BASIC_CRIT, "basic_crit") 
            + self._extract(StatsPatterns.EXTRA_CRIT, "extra_crit")
        )
        
        total_acc = (
            self._extract(StatsPatterns.BASIC_ACCURACY, "basic_accuracy") 
        ...
        return UserProfile(
            ...
            character_lvl=self._extract(
                CommonPatterns.CHARACTER_LVL, 
                "character_lvl"
            ),
            paragon_lvl=self._extract(
                CommonPatterns.PARAGON_LVL, 
                "paragon_lvl"
            ),
            ...
            character_exp=self._extract(
                ResourcePatterns.CHARACTER_EXP, 
                "character_exp"
            ),
